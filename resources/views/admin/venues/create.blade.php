...
<div class="mb-4 sm:mb-6">
    <div class="flex items-center justify-between">
        <div>
            <h1 class="text-2xl sm:text-3xl font-bold">Add Venue</h1>
            <p class="text-neutral-600 mt-1 text-sm sm:text-base">Create a new venue for workshops</p>
        </div>
    ...
    <form method="POST" action="{{ route('admin.venues.store') }}" class="space-y-6">
        ...
        <div>
            ...
            <textarea name="address" id="address" rows="2"
                class="w-full px-4 py-2 border border-neutral-300 rounded-lg focus:ring-2 focus:ring-primary @error('address') border-red-500 @enderror">{{ old('address') }}</textarea>
        ...
        <div>
            ...
            <p class="text-xs text-neutral-500 mb-2">Select which trades can be offered at this venue. Workshops can then assign venue and trades.</p>
            <div class="space-y-2 border border-neutral-200 rounded-lg p-3 max-h-48 overflow-y-auto">
                @forelse($trades as $trade)
                <label class="flex items-center gap-2">
                    <input type="checkbox" name="trades[]" value="{{ $trade->id }}" {{ in_array($trade->id, old('trades', [])) ? 'checked' : '' }} class="rounded">
