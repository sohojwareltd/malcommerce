...
<div class="bg-white rounded-lg shadow-md p-4 sm:p-6">
    <form method="POST" action="{{ route('admin.users.store') }}" class="space-y-4 sm:space-y-6">
        ...
        <div class="grid grid-cols-1 md:grid-cols-2 gap-4 sm:gap-6">
            ...
            <!-- Role -->
            <div>
                <label for="role_id" class="block text-sm font-medium text-neutral-700 mb-2">Role <span class="text-red-500">*</span></label>
                <select id="role_id" name="role_id" required
                    class="w-full px-4 py-2 border border-neutral-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary text-sm sm:text-base @error('role_id') border-red-500 @enderror">
                ...
                @error('role_id')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                <p class="mt-1 text-xs text-neutral-500">Role determines what features this user can access</p>
            </div>
        </div>
