...
<form action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="bg-white rounded-lg shadow-md p-6" onsubmit="updateDeliveryOptionsJson(); return true;">
    ...
    <div id="tab-variations" class="tab-panel hidden">
        <div class="space-y-6">
            ...
            <div class="border border-neutral-200 rounded-lg p-4 bg-neutral-50/40">
                <div class="flex items-center justify-between gap-3 mb-3">
                    <div>
                        <h3 class="text-base font-semibold text-neutral-900">Product Variations</h3>
                        <p class="text-xs text-neutral-500">You can also add variants manually or edit generated ones.</p>
                    </div>
...
function addAttributeGroup() {
    ...
    // Default visibility for first added rows (none yet)
}
...
function addAttributeValueRow(groupIndex) {
    ...
    // Keep hex input synced with color picker
...
function generateVariantsFromAttributes() {
    ...
    // Existing variants dedup (based on hidden variants[*][attributes])
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    if (Array.isArray(oldVariants) && oldVariants.length > 0) {
        oldVariants.forEach((variant) => addVariantRow(variant));
    }
