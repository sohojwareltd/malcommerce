            ...
            <div class="flex items-center justify-between">
                <div class="flex items-center gap-6">
                    ...
                    <div class="relative flex-shrink-0">
                        @if(Auth::user()->photo)
                            <img src="{{ Storage::disk('public')->url(Auth::user()->photo) }}" alt="{{ Auth::user()->name }}" class="w-24 h-24 rounded-full object-cover border-4 border-white shadow-lg ring-4 ring-primary/20">
                        @else
                            <div class="w-24 h-24 rounded-full bg-white flex items-center justify-center border-4 border-white shadow-lg ring-4 ring-primary/20">
                                <svg class="w-12 h-12 text-primary" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                                    <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M16 7a4 4 0 11-8 0 4 4 0 018 0zM12 14a7 7 0 00-7 7h14a7 7 0 00-7-7z"></path>
                                </svg>
                            </div>
                        @endif
                        <div class="absolute bottom-0 right-0 bg-green-500 border-4 border-white rounded-full w-6 h-6"></div>
                    </div>
                    <!-- User Info -->
                    <div class="text-white">
                        ...
                        <div class="flex items-center gap-4">
                            <div class="flex items-center gap-2 bg-white/20 backdrop-blur-sm rounded-lg px-3 py-1.5">
                                ...
                                <span class="text-sm font-medium">Partner Code</span>
                                <span class="font-mono font-bold text-lg">{{ Auth::user()->affiliate_code }}</span>
                            </div>
                            @if(Auth::user()->phone)
                            <div class="flex items-center gap-2 bg-white/20 backdrop-blur-sm rounded-lg px-3 py-1.5">
                                <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                ...
                <!-- Edit Button -->
                <a href="{{ route('sponsor.profile.edit') }}" class="bg-white text-primary px-6 py-3 rounded-xl hover:bg-neutral-50 transition font-semibold shadow-lg hover:shadow-xl flex items-center gap-2 transform hover:scale-105">
...
<div class="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-6 mb-8">
    ...
    <!-- Income Card -->
    <div class="bg-green-500 rounded-xl shadow-lg p-6 text-white transform hover:scale-105 transition duration-200">
        <div class="flex items-center justify-between mb-4">
