...
<div class="mb-6 flex items-center justify-between">
    <div>
        <h1 class="text-3xl font-bold">Edit Referral User</h1>
        <p class="text-neutral-600 mt-1">Update information for {{ $referral->name }}</p>
    </div>
...
<div class="bg-white rounded-lg shadow-md p-6 max-w-2xl">
    <form action="{{ route('sponsor.users.update', $referral) }}" method="POST" enctype="multipart/form-data">
        ...
        <div class="mb-6">
            ...
            <div class="flex flex-col md:flex-row items-start md:items-center gap-6">
                <div id="photo-preview-container" class="flex-shrink-0">
                    @if($referral->photo)
                        <img id="photo-preview" src="{{ Storage::disk('public')->url($referral->photo) }}" alt="Profile Photo" class="w-32 h-32 rounded-full object-cover border-4 border-primary shadow-lg transition-all duration-300">
                    @else
        ...
        <div class="mb-6">
            ...
            <input type="email" name="email" id="email" value="{{ old('email', $referral->email) }}"
        ...
        <div class="mb-6">
            ...
            <input type="text" value="{{ $referral->phone }}" readonly
                class="w-full px-4 py-2 border border-neutral-300 rounded-lg bg-neutral-100 text-neutral-500 cursor-not-allowed">
            <p class="mt-1 text-xs text-neutral-500">Phone number cannot be changed</p>
        </div>
        ...
        <div class="mb-6">
            ...
            <textarea name="address" id="address" rows="4"
                class="w-full px-4 py-2 border border-neutral-300 rounded-lg focus:ring-2 focus:ring-primary focus:border-primary @error('address') border-red-500 @enderror">{{ old('address', $referral->address) }}</textarea>
            @error('address')
                <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
            @enderror
        </div>
        ...
        <div class="mb-6">
            <label for="affiliate_code" class="block text-sm font-medium text-neutral-700 mb-2">Affiliate Code</label>
            <input type="text" value="{{ $referral->affiliate_code }}" readonly
                class="w-full px-4 py-2 border border-neutral-300 rounded-lg bg-neutral-100 text-neutral-500 font-mono cursor-not-allowed">
            <p class="mt-1 text-xs text-neutral-500">Affiliate code cannot be changed</p>
        </div>
